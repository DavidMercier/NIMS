...
if gui.variables.val2 ~= 1
    if gui.variables.y_axis == 3
        ...
        if gui.variables.val2 ~= 4
            gui.results.Ef_red_sol0 = str2double(get(gui.handles.value_youngfilm1_GUI, 'String'));    % Make a starting guess at the solution (Ef in GPa)
    ...
    elseif gui.variables.y_axis == 4
        ...
        elseif gui.variables.val2 == 5 % Bec et al. (2006)
            gui.data.Ef_red = 1e-9 * (((pi.*gui.results.ac.^2)./(gui.results.t_corr)) .* ...
        ...
        elseif gui.variables.val2 == 6 % Hay et al. (2011)
            ...
            B_Hay  = mus-(((F.*gui.data.phigao0.^2)-gui.data.phigao0+1).*mueq);
...
elseif gui.variables.val2 == 1 || gui.variables.y_axis == 5 || gui.variables.val5 == 1 || gui.variables.y_axis == 2 % No Bilayer Model
