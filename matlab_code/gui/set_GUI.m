%% Copyright 2014 MERCIER David
function handles = set_GUI
%% Definition of the GUI and buttons
g = guidata(gcf);

%% Title of the GUI
handles.title_GUI_1 = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.31 0.96 0.6 0.04],...
    'String', ['Extraction of mechanical properties of thin film(s) ',...
    'on substrate by conical nanoindentation.'],...
    'FontWeight', 'bold',...
    'FontSize', 12,...
    'HorizontalAlignment', 'center',...
    'ForegroundColor', 'red');

handles.title_GUI_2 = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.31 0.93 0.6 0.03],...
    'String', strcat('Version_', ...
    g.config.version_toolbox, ' - Copyright 2014 MERCIER David'),...
    'FontSize', 10);

set([handles.title_GUI_1, handles.title_GUI_2], ...
    'FontWeight', 'bold',...
    'HorizontalAlignment', 'center',...
    'ForegroundColor', 'red')

%% Date / Time
handles.date_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'String', datestr(datenum(clock),'mmm.dd,yyyy HH:MM'),...
    'Position', [0.92 0.975 0.075 0.02]);

%% Buttons to browse in files
handles.pm_set_indenter = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popupmenu',...
    'Position', [0.018 0.95 0.08 0.04],...
    'String', {'Agilent MTS';'Hysitron'},...
    'Value', 1,...
    'Callback', '');

handles.opendata_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'pushbutton',...
    'Position', [0.1 0.93 0.05 0.06],...
    'String', 'Select file',...
    'FontSize', 10,...
    'FontWeight','bold',...
    'BackgroundColor', [0.745 0.745 0.745],...
    'Callback', 'openfile');

handles.opendata_str_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'edit',...
    'Position', [0.155 0.93 0.122 0.06],...
    'String', g.config.data.data_path,...
    'FontSize', 8,...
    'BackgroundColor', [0.9 0.9 0.9],...
    'HorizontalAlignment', 'left');

%% Units
% Unit definition for the load
handles.title_unitLoad_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.018 0.90 0.025 0.02],...
    'String', 'Load :',...
    'HorizontalAlignment', 'left');

handles.unitLoad_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popupmenu',...
    'Position', [0.043 0.90 0.04 0.02],...
    'String', {'nN';'uN';'mN'},...
    'Value', 3,...
    'Callback', 'get_and_plot');

% Unit definition for the displacement
handles.title_unitDisp_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.093 0.90 0.055 0.02],...
    'String', 'Displacement :',...
    'HorizontalAlignment', 'left');

handles.unitDisp_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popupmenu',...
    'Position', [0.148 0.90 0.04 0.02],...
    'String', {'nm';'um';'mm'},...
    'Value', 1,...
    'Callback', 'get_and_plot');

% Unit definition for the stiffness
handles.title_unitStif_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.198 0.90 0.04 0.02],...
    'String', 'Stiffness :',...
    'HorizontalAlignment', 'left');

handles.unitStif_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popupmenu',...
    'Position', [0.238 0.90 0.04 0.02],...
    'String', {'N/m';'mN/um';'mN/nm'},...
    'Value', 1,...
    'Callback', 'get_and_plot');

%% Choice of the indenter (only conical indenters...)
handles.title_indentertype_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.86 0.12 0.02],...
    'String', 'Type of indenter',...
    'HorizontalAlignment', 'left');

for ii = 1:1:length(g.config.indenter.Indenter_IDs)
    if strcmp(g.config.indenter.Indenter_ID, g.config.indenter.Indenter_IDs(ii)) == 1
        ind_Indenter = ii;
    end
end

handles.value_indentertype_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popupmenu',...
    'Position', [0.02 0.84 0.12 0.02],...
    'String', g.config.indenter.Indenter_IDs,...
    'Value', ind_Indenter,...
    'Callback', 'refresh_indenters_GUI');

% Indenter tip - Creation of button group
handles.bg_indenter_tip_GUI = uibuttongroup('Parent', gcf,...
    'Position', [0.02 0.73 0.255 0.1]);

%% Choice of the material indenter
handles.title_indentermaterial_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.155 0.86 0.12 0.02],...
    'String', 'Material of indenter',...
    'HorizontalAlignment', 'left');

handles.value_indentermaterial_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popupmenu',...
    'Position', [0.155 0.84 0.12 0.02],...
    'String', g.config.indenter.Indenter_materials,...
    'Value', 1,...
    'Callback', 'refresh_indenters_GUI');

set(handles.value_indentermaterial_GUI, 'value', ...
    find(cell2mat(strfind(g.config.indenter.Indenter_materials, ...
    g.config.indenter.Indenter_material))));

%% Number of thin films
handles.title_numthinfilm_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.6975 0.1 0.02],...
    'String', 'Number of thin films',...
    'HorizontalAlignment', 'left');

handles.value_numthinfilm_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popup',...
    'Position', [0.02 0.6775 0.1 0.02],...
    'String', '0|1|2|3',...
    'Value', 4,...
    'Callback', 'refresh_indenters_GUI');

%% Convention
handles.plot_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'pushbutton',...
    'Position', [0.122 0.6725 0.153 0.045],...
    'String', 'Convention',...
    'FontSize', 12,...
    'BackgroundColor', [0.745 0.745 0.745]);
if g.config.licenceIma_Flag
    set(handles.plot_GUI, ...
        'Callback', 'figure; imshow(''convention_multilayer.png'');');
else
    set(handles.plot_GUI, ...
        'Callback', 'web(''http://nims.readthedocs.org/en/latest/_images/convention_multilayer.png'',''-browser'');');
end

%% Properties of each layers (thickness, Young's modulus, Poisson's ratio)
handles.bg_film2_properties_GUI = uibuttongroup(...
    'Parent', gcf, ...
    'Position', [0.02 0.6175 0.255 0.0375]);
handles.bg_film1_properties_GUI = uibuttongroup(...
    'Parent', gcf, ...
    'Position', [0.02 0.5800 0.255 0.0375]);
handles.bg_film0_properties_GUI = uibuttongroup(...
    'Parent', gcf, ...
    'Position', [0.02 0.5425 0.255 0.0375]);
handles.bg_substrat_properties_GUI = uibuttongroup(...
    'Parent', gcf, ...
    'Position', [0.02 0.5050 0.255 0.0375]);

%% Definition of the minimum/maximum depth
handles.title_mindepth_prop_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.465 0.07 0.02],...
    'String', 'Minimum depth :',...
    'HorizontalAlignment', 'left');

handles.value_mindepth_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'edit',...
    'Position', [0.08 0.465 0.03 0.02],...
    'String', '',...
    'Callback', 'get_and_plot');

handles.unit_mindepth_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.11 0.465 0.02 0.02],...
    'String', 'nm',...
    'HorizontalAlignment', 'center');

handles.title_maxdepth_prop_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.445 0.07 0.02],...
    'String', 'Maximum depth :',...
    'HorizontalAlignment', 'left');

handles.value_maxdepth_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'edit',...
    'Position', [0.08 0.445 0.03 0.02],...
    'String', '',...
    'Callback', 'get_and_plot');

handles.unit_maxdepth_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.11 0.445 0.02 0.02],...
    'String', 'nm',...
    'HorizontalAlignment', 'center');

%% Frame compliance correction
handles.title_frame_compliance_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.14 0.465 0.07 0.02],...
    'String', 'Frame compliance :',...
    'HorizontalAlignment', 'left');

handles.value_frame_compliance_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'edit',...
    'Position', [0.21 0.465 0.035 0.02],...
    'String', '0',...
    'Callback', 'get_and_plot');

handles.unit_frame_compliance_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.245 0.465 0.03 0.02],...
    'String', 'um/mN',...
    'HorizontalAlignment', 'center');

%% Parameter to plot
handles.title_param2plotinxaxis_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.415 0.12 0.02],...
    'String', 'X axis',...
    'HorizontalAlignment', 'left');

handles.value_param2plotinxaxis_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popup',...
    'Position', [0.02 0.395 0.12 0.02],...
    'String', 'Displ.|Cont.rad./Thick.|Displ./Thick.',...
    'Value', 1,...
    'Callback', 'get_and_plot');

handles.title_param2plotinyaxis_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.155 0.415 0.12 0.02],...
    'String', 'Y axis',...
    'HorizontalAlignment', 'left');

handles.value_param2plotinyaxis_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popup',...
    'Position', [0.155 0.395 0.12 0.02],...
    'String', {'Load', ...
    'Stiffness', ...
    'Load over Stiffness squared', ...
    'Red. Young''s modulus (sample)', ...
    'Red. Young''s modulus (film)', ...
    'Hardness', ...
    'Red. Young''s modulus Guillonneau', ...
    'Hardness Guillonneau'},...
    'Value', 1,...
    'Callback', 'get_and_plot');

%% Choice of the model for contact displacement calculation
handles.title_modeldisp_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.36 0.12 0.02],...
    'String', 'Model for contact displacement',...
    'HorizontalAlignment', 'left');

handles.value_modeldisp_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popup',...
    'Position', [0.02 0.34 0.12 0.02],...
    'String', 'Doerner&Nix|Oliver&Pharr|Loubet',...
    'Value', 2,...
    'Callback', 'get_and_plot');

%% Correction parameter in Young's modulus
handles.title_corr_King_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.155 0.36 0.12 0.02],...
    'String', 'Correction to apply',...
    'HorizontalAlignment', 'left');

handles.popup_corr_King_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popup',...
    'Position', [0.155 0.34 0.12 0.02],...
    'String', 'beta King1987|beta Hay1999',...
    'Value', 1,...
    'Callback', 'get_and_plot');

%% Choice of the model to fit load-displacement curves
handles.title_model_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.305 0.12 0.02],...
    'HorizontalAlignment', 'left');

handles.value_model_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popup',...
    'Position', [0.02 0.285 0.12 0.02],...
    'String', '',...
    'Value', 1,...
    'Callback', 'get_and_plot');

%% LoadDisp / Bilayer / Multilayer models

% List for load-displacement models
handles.title_load_disp_model = 'Load-Disp. Model';

handles.list_load_disp_model = {...
    'No_model',...
    'Loubet', ...
    'Hainsworth'};

set(handles.title_model_GUI, 'String', handles.title_load_disp_model);
set(handles.value_model_GUI, 'String', handles.list_load_disp_model);

% List for stiffness models
handles.title_stiffness_model = 'Stiffness Model';

handles.list_stiffness_model = {...
    'No_model',...
    'Linear', ...
    '2nd degree polynomial'};

% List for stiffness models
handles.title_loadOverstiffnessSquared_model = 'Load/(Stiffness squared) Model';

handles.list_loadOverstiffnessSquared_model = {....
    'No_model',...
    'Linear'};

% Titles popup menus
handles.title_bilayermodel = 'Bilayer Model';
handles.title_multilayermodel = 'Multilayer Model';

% List for elastic bilayer/multilayer models
handles.list_bilayermodel = {...
    'No_Bilayer_Model', ...
    'Doerner&Nix',...
    'Doerner&Nix_King',...
    'Doerner&Nix_Saha',...
    'Doerner&Nix_Chen',...
    'Gao_etal.',...
    'Bec_etal.',...
    'Hay_etal.',...
    'Jung_etal.',...
    'Perriot_etal.',...
    'Mencik_etal._linear',...
    'Mencik_etal._exponential',...
    'Mencik_etal._reciprocal_exp.'};

handles.list_multilayermodel = {...
    'No_Multilayer_Model', ...
    'Mercier_etal.'};

% List for plastic bilayer/multilayer models
handles.list_bilayermodel_plastic = {...
    'No_Model', ...
    'KaoByrne', ...
    'BhattacharyaNix_HCSS', ...
    'BhattacharyaNix_SCHS', ...
    'Han_etal',...
    'Mercier_etal'};

handles.list_multilayermodel_plastic = {...
    'No_Model', ...
    'Han_etal',...
    'Mercier_etal'};

%% Choice of the elastic model used in Saha's method to calculate the hardness
handles.title_models_Saha_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.155 0.305 0.12 0.02],...
    'String', 'Elastic model',...
    'HorizontalAlignment', 'left');

handles.value_models_Saha_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popup',...
    'Position', [0.155 0.285 0.12 0.02],...
    'String', 'model1|model2',...
    'Value', 2,...
    'Callback', 'get_and_plot');

%% CSM correction
handles.cb_CSM_corr_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'checkbox',...
    'Position', [0.02 0.25 0.18 0.02],...
    'String', 'CSM correction (only valide for Berkovich indenters)',...
    'Value', 0,...
    'Callback', 'get_and_plot');

%% Film thickness correction
handles.cb_corr_thickness_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'checkbox',...
    'Position', [0.02 0.225 0.18 0.02],...
    'String', 'Thickness correction (Mencik1997 / Li2010)',...
    'Value', 0,...
    'Callback', 'get_and_plot');

handles.title_neta_corr_thickness_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.215 0.225 0.03 0.02],...
    'String', char(951),...
    'HorizontalAlignment', 'left');

handles.value_neta_corr_thickness_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'edit',...
    'Position', [0.23 0.225 0.03 0.02],...
    'String', '0.3',...
    'Callback', 'get_and_plot');

%% Options of the plot
handles.cb_log_plot_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'checkbox',...
    'Position', [0.02 0.18 0.04 0.03],...
    'String', 'Log',...
    'Value', 0,...
    'Callback', 'get_and_plot');

handles.cb_grid_plot_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'checkbox',...
    'Position', [0.06 0.18 0.04 0.03],...
    'String', 'Grid',...
    'Value', 1,...
    'Callback', 'get_and_plot');

handles.cb_SD_plot_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'checkbox',...
    'Position', [0.10 0.18 0.04 0.03],...
    'String', 'SD',...
    'Value', 1,...
    'Callback', 'get_and_plot');

handles.cb_lines_plot_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'checkbox',...
    'Position', [0.14 0.18 0.08 0.03],...
    'String', 'Interface Line(s)',...
    'Value', 1,...
    'Callback', 'get_and_plot');

%% Location of the legend
handles.title_legend_location = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.15 0.12 0.02],...
    'String', 'Legend location',...
    'HorizontalAlignment', 'left');

handles.value_legend_location = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'popup',...
    'Position', [0.02 0.13 0.12 0.02],...
    'String', listLocationLegend,...
    'Value', 6,...
    'Callback', 'get_and_plot');

%% Plot of the residuals
handles.pb_residual_plot_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'pushbutton',...
    'Position', [0.155 0.125 0.12 0.04],...
    'String', 'Residuals',...
    'Visible', 'off',...
    'Value', 0,...
    'Callback', 'plot_selection(2)');

%% Get values from plot
handles.pb_get_values_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'pushbutton',...
    'Position', [0.02 0.08 0.1 0.03],...
    'String', 'Get x and y values',...
    'Callback', 'plot_get_values');

handles.title_x_values_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.06 0.03 0.02],...
    'String', 'X value :',...
    'HorizontalAlignment', 'left');

handles.value_x_values_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'edit',...
    'Position', [0.05 0.06 0.03 0.02],...
    'String', '');

handles.title_y_values_prop_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'text',...
    'Position', [0.02 0.04 0.03 0.02],...
    'String', 'Y value :',...
    'HorizontalAlignment', 'left');

handles.value_y_values_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'edit',...
    'Position', [0.05 0.04 0.03 0.02],...
    'String', '');

%% Plot configuration
handles.AxisPlot_GUI = axes('Parent', gcf,...
    'Position', [0.33 0.1 0.65 0.75]);

set(gcf,'CurrentAxes', handles.AxisPlot_GUI);

%% Others buttons
% Python for FEM (Abaqus)
handles.python4fem = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'pushbutton',...
    'Position', [0.155 0.075 0.12 0.04],...
    'String', 'FEM',...
    'Callback', 'python4abaqus');

% Save
handles.save_GUI = uicontrol('Parent', gcf,...
    'Units', 'normalized',...
    'Style', 'pushbutton',...
    'Position', [0.155 0.03 0.12 0.04],...
    'String', 'SAVE',...
    'Callback', 'export_data_to_YAML_file');

% % Quit
% handles.quit_GUI = uicontrol('Parent', gcf,...
%     'Units', 'normalized',...
%     'Style', 'pushbutton',...
%     'Position', [0.02 0.035 0.12 0.04],...
%     'String', 'QUIT',...
%     'Callback', 'close(gcf);clear all');

set([handles.python4fem, handles.save_GUI],... %, handles.quit_GUI
    'FontSize', 12,...
    'BackgroundColor', [0.745 0.745 0.745]);

end